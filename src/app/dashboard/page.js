...
// Pre-defined list of tips to display
const PET_CARE_TIPS = [
  {
...
export default async function DashboardHomePage() {
  ...
  if (email && token) {
    try {
      ...
      // 2. Fetch adoption requests sent by the user
      sentRequests = await getMyRequests(email, token) || [];

      // 3. Fetch adoption requests received on user listings
      if (listings.length > 0) {
        ...
        receivedRequests = results.flat().filter(Boolean);
      }
    } catch (error) {
  ...
  const successRate = totalListings > 0 ? Math.round((adoptedListings / totalListings) * 100) : 0;

  // Get a random tip based on current date so it remains stable for a day, or pseudo-random on page loads
  const tipIndex = typeof window === "undefined" 
    ? Math.floor((new Date().getDate() + new Date().getMonth()) % PET_CARE_TIPS.length)
    : Math.floor(Math.random() * PET_CARE_TIPS.length);
  const dailyTip = PET_CARE_TIPS[tipIndex];

  // Dynamic greeting warning or action text
  let greetingMessage = "👋 Ready to make a difference? Add a new pet listing or explore pets to adopt!";
  if (pendingReceived > 0) {
    greetingMessage = `🐾 You have ${pendingReceived} pending adoption request${pendingReceived > 1 ? 's' : ''} waiting for your review. Let's find them a loving home!`;
  } else if (activeListings > 0) {
    greetingMessage = "✨ Your listed pets are doing great! Let's check if any new friends are interested.";
  } else if (totalSent > 0) {
    greetingMessage = "💛 Your adoption applications are submitted. We hope you connect with your dream pet soon!";
  }

  return (
    <div className="space-y-6 bg-background text-foreground min-h-screen">
      {/* Background ambient blurs — fixed so they don't affect layout */}
      ...
      <div className="relative overflow-hidden rounded-3xl border border-border/80 bg-card/60 backdrop-blur-xl p-5 sm:p-7 shadow-sm flex flex-col xl:flex-row items-start xl:items-center justify-between gap-5 transition-all duration-300 hover:shadow-md hover:border-primary/20">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 z-10 min-w-0 w-full xl:w-auto">
          ...
          <div className="min-w-0">
            ...
            <h1 className="text-xl sm:text-2xl lg:text-3xl font-extrabold tracking-tight text-foreground mt-2" style={{ fontFamily: "var(--font-poppins)" }}>
              Welcome back, <span className="text-primary">{user?.name || "Pet Lover"}</span>!
            </h1>
        ...
        {/* Quick listing stats bubble — show at xl to avoid crowding the lg 4-col grid breakpoint */}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
        ...
        <div className="p-4 sm:p-5 rounded-2xl border border-border bg-card shadow-sm hover:shadow-md hover:border-primary/20 transition-all duration-300 flex flex-col justify-between group">
          <div className="flex items-start justify-between">
            <div className="min-w-0">
              <p className="text-xs sm:text-sm font-medium text-muted-foreground">My Pet Listings</p>
              <p className="text-2xl sm:text-3xl font-extrabold mt-1.5 text-foreground">{totalListings}</p>
            </div>
          ...
          <div className="mt-3 pt-3 border-t border-border/50 flex items-center justify-between text-xs">
            <span className="text-muted-foreground flex items-center gap-1">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" /> {activeListings} Avail.
            </span>
            <span className="text-muted-foreground flex items-center gap-1">
              <span className="w-1.5 h-1.5 rounded-full bg-primary" /> {adoptedListings} Adopted
            </span>
          </div>
        </div>
        ...
        <div className="p-4 sm:p-5 rounded-2xl border border-border bg-card shadow-sm hover:shadow-md hover:border-primary/20 transition-all duration-300 flex flex-col justify-between group">
          <div className="flex items-start justify-between">
            <div className="min-w-0">
              <p className="text-xs sm:text-sm font-medium text-muted-foreground">Incoming Requests</p>
              <div className="flex items-baseline gap-1.5 mt-1.5 flex-wrap">
                <p className="text-2xl sm:text-3xl font-extrabold text-foreground">{totalReceived}</p>
                {pendingReceived > 0 && (
          ...
          <div className="mt-3 pt-3 border-t border-border/50 flex items-center justify-between text-xs text-muted-foreground">
            ...
            <span>{totalReceived - pendingReceived - approvedReceived} Rejected</span>
          </div>
        </div>
        ...
        <div className="p-4 sm:p-5 rounded-2xl border border-border bg-card shadow-sm hover:shadow-md hover:border-primary/20 transition-all duration-300 flex flex-col justify-between group">
          <div className="flex items-start justify-between">
            <div className="min-w-0">
              <p className="text-xs sm:text-sm font-medium text-muted-foreground">My Sent Requests</p>
              <p className="text-2xl sm:text-3xl font-extrabold mt-1.5 text-foreground">{totalSent}</p>
            </div>
            <div className="p-2 sm:p-3 rounded-xl bg-primary/10 text-primary group-hover:scale-110 transition-transform duration-300 shrink-0 ml-2">
              <Send size={18} />
            </div>
          ...
          <div className="mt-3 pt-3 border-t border-border/50 flex items-center justify-between text-xs">
            <span className="text-muted-foreground flex items-center gap-1">
              <span className="w-1.5 h-1.5 rounded-full bg-amber-400" /> {pendingSent} Pending
            </span>
            <span className="text-muted-foreground flex items-center gap-1">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" /> {approvedSent} Approved
            </span>
          </div>
        </div>
        ...
        <div className="p-4 sm:p-5 rounded-2xl border border-border bg-card shadow-sm hover:shadow-md hover:border-primary/20 transition-all duration-300 flex flex-col justify-between group">
          <div className="flex items-start justify-between">
            <div className="min-w-0">
              <p className="text-xs sm:text-sm font-medium text-muted-foreground">Adoption Rate</p>
              <p className="text-2xl sm:text-3xl font-extrabold mt-1.5 text-foreground">{successRate}%</p>
            </div>
            <div className="p-2 sm:p-3 rounded-xl bg-primary/10 text-primary group-hover:scale-110 transition-transform duration-300 shrink-0 ml-2">
              <Sparkles size={18} />
            </div>
          ...
          <div className="mt-4 pt-3 border-t border-border/50">
            ...
            <div className="w-full h-1.5 bg-muted rounded-full overflow-hidden">
              <div className="h-full bg-primary rounded-full transition-all duration-500" style={{ width: `${successRate}%` }} />
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
          ...
          <div className="p-4 sm:p-5 rounded-2xl border border-border bg-card shadow-sm space-y-4">
            <div className="flex items-start justify-between border-b border-border pb-4 gap-3">
              <div className="min-w-0">
                ...
                <p className="text-xs text-muted-foreground mt-0.5">Requests for your listed pets</p>
              </div>
              <Link href="/dashboard/my-listings" className="text-xs font-semibold text-primary hover:underline flex items-center gap-1 shrink-0">
            ...
            {loadingError ? (
              ...
              <div className="text-center py-10 px-4 space-y-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1 max-w-md mx-auto">
                    When visitors submit requests to adopt the pets you have listed, they will appear here with contact and status info.
                  </p>
                </div>
                <Link href="/dashboard/add-pet" className="inline-flex items-center gap-1.5 bg-primary/10 border border-primary/20 text-primary text-xs font-semibold px-4 py-2 rounded-full hover:bg-primary hover:text-white transition duration-300">
              ...
              <div className="space-y-3">
                {receivedRequests.slice(0, 3).map((request) => (
                  <div key={request._id} className="p-3 sm:p-4 rounded-xl border border-border bg-muted/20 hover:border-primary/20 transition duration-200">
                    {/* Top row: avatar + name + status */}
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-3 min-w-0">
                        {request.petImage ? (
                          <div className="relative w-10 h-10 rounded-xl overflow-hidden border border-border shrink-0">
                            <Image src={request.petImage} alt={request.petName || "Pet"} fill className="object-cover" />
                          </div>
                        ) : (
                      ...
                      <span className={`px-2 py-0.5 rounded-full text-xs font-semibold capitalize border shrink-0 ${
                    ...
                    {/* Bottom row: pet name + date + action */}
                    <div className="flex items-center justify-between mt-2.5 pt-2.5 border-t border-border/50 gap-3">
                      <div className="flex items-center gap-3 text-xs text-muted-foreground min-w-0">
                        <span className="font-medium text-primary truncate">🐾 {request.petName}</span>
                        <span className="flex items-center gap-1 shrink-0"><Calendar size={11} /> {request.pickupDate || "No date"}</span>
                      </div>
                      <Link href="/dashboard/my-listings" className="shrink-0">
                        <button className="text-xs font-semibold border border-border hover:bg-muted text-foreground px-2.5 py-1 rounded-lg transition duration-200">
                          Action
                        </button>
                      </Link>
                    </div>
                ...
                {receivedRequests.length > 3 && (
                  <p className="text-xs text-center text-muted-foreground pt-1">
                    +{receivedRequests.length - 3} more. View listings to see all.
                  </p>
                )}
              </div>
            )}
          ...
          <div className="p-4 sm:p-5 rounded-2xl border border-border bg-card shadow-sm space-y-4">
            <div className="flex items-start justify-between border-b border-border pb-4 gap-3">
              <div className="min-w-0">
                ...
                <p className="text-xs text-muted-foreground mt-0.5">Track your requests on Pawly</p>
              </div>
              <Link href="/dashboard/my-requests" className="text-xs font-semibold text-primary hover:underline flex items-center gap-1 shrink-0">
            ...
            {loadingError ? (
              ...
              <div className="text-center py-10 px-4 space-y-4">
                ...
                <div>
                  <p className="text-sm font-semibold text-foreground">You haven&apos;t applied to adopt yet</p>
                  <p className="text-xs text-muted-foreground mt-1 max-w-md mx-auto">
                    Find your perfect companion. Browse through our listings of sweet pets looking for a happy home and submit your adoption request!
                  </p>
                </div>
                <Link href="/all-pets" className="inline-flex items-center gap-1.5 bg-primary/10 border border-primary/20 text-primary text-xs font-semibold px-5 py-2.5 rounded-full hover:bg-primary hover:text-white transition duration-300 shadow-sm">
                  ...
                  <div key={request._id} className="p-3 sm:p-4 rounded-xl border border-border bg-muted/20 hover:border-primary/20 transition duration-200">
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          <h4 className="text-sm font-bold text-foreground truncate">Adopt {request.petName}</h4>
                          <p className="text-xs text-muted-foreground">{new Date(request.createdAt).toISOString().split("T")[0]}</p>
                        </div>
                      </div>
                      <span className={`px-2 py-0.5 rounded-full text-xs font-semibold capitalize border shrink-0 ${
                        request.status === "approved"
                    ...
                    {/* Bottom row: pickup + action */}
                    <div className="flex items-center justify-between mt-2.5 pt-2.5 border-t border-border/50 gap-3">
                      <span className="flex items-center gap-1 text-xs text-muted-foreground">
                        <Calendar size={11} /> Pickup: {request.pickupDate || "Pending"}
                      </span>
                      <Link href={`/all-pets/${request.petId}`} className="shrink-0">
                        <button className="text-xs font-semibold border border-border hover:bg-muted text-foreground px-2.5 py-1 rounded-lg transition duration-200">
                          Details
                        </button>
                      </Link>
        ...
        <div className="space-y-5">
          ...
          {/* Quick Actions Panel */}
          <div className="p-4 sm:p-5 rounded-2xl border border-border bg-card shadow-sm space-y-4">
            <h2 className="text-base font-bold text-foreground border-b border-border pb-3">Quick Actions</h2>
            
            <div className="space-y-3">
              {/* Add Pet */}
              <Link href="/dashboard/add-pet" className="group block p-4 rounded-xl border border-border/80 bg-muted/10 hover:bg-primary/5 hover:border-primary/30 transition-all duration-200">
                <div className="flex items-center gap-3">
                  <div className="p-2.5 rounded-lg bg-primary/10 text-primary group-hover:scale-110 transition-transform">
                    <Plus size={18} />
                  </div>
                  <div>
                    <h3 className="text-sm font-bold text-foreground">Add New Pet</h3>
                    <p className="text-xs text-muted-foreground mt-0.5">List a pet for adoption</p>
                  </div>
                </div>
              </Link>

              {/* View Listings */}
              <Link href="/dashboard/my-listings" className="group block p-4 rounded-xl border border-border/80 bg-muted/10 hover:bg-primary/5 hover:border-primary/30 transition-all duration-200">
                <div className="flex items-center gap-3">
                  <div className="p-2.5 rounded-lg bg-primary/10 text-primary group-hover:scale-110 transition-transform">
                    <PawPrint size={18} />
                  </div>
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">Manage your active pets</p>
                  </div>
                </div>
              </Link>

              {/* Check Requests */}
              <Link href="/dashboard/my-requests" className="group block p-4 rounded-xl border border-border/80 bg-muted/10 hover:bg-primary/5 hover:border-primary/30 transition-all duration-200">
                <div className="flex items-center gap-3">
                  <div className="p-2.5 rounded-lg bg-primary/10 text-primary group-hover:scale-110 transition-transform">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">Track your applications</p>
                  </div>
                </div>
              </Link>
            </div>
          ...
          <div className="p-4 sm:p-5 rounded-2xl border border-primary/20 bg-primary/5 shadow-sm relative overflow-hidden group">
            ...
            <div className="space-y-3 z-10 relative">
              ...
              <h3 className="text-base font-bold text-foreground">{dailyTip.title}</h3>
              <p className="text-xs text-muted-foreground leading-relaxed">{dailyTip.content}</p>
              <p className="text-[10px] font-medium text-primary/80 pt-1.5">— {dailyTip.author}</p>
            </div>
          </div>
          ...
          <div className="p-4 sm:p-5 rounded-2xl border border-border bg-card shadow-sm space-y-3">
            ...
            <div className="space-y-3 text-xs leading-relaxed text-muted-foreground">
              <p>
                Adoption listings with detailed descriptions and at least 3 photos receive up to <span className="font-semibold text-primary">60% more adoption requests</span>.
              </p>
              <p>
                Prompt responses to requests within <span className="font-semibold text-primary">24 hours</span> increase successful placements by over <span className="font-semibold text-primary">2 times</span>.
              </p>
            </div>
          </div>
      ...
      <footer className="mt-2 pt-5 border-t border-border/60 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-muted-foreground">
        <p>© 2026 Pawly Inc. Supporting pet care &amp; smooth adoptions.</p>
        <p className="flex items-center gap-1.5">
          <Sparkles size={12} className="text-primary animate-pulse" /> Keep creating beautiful stories together.
        </p>
      </footer>
    </div>
  );
